...
export class AppComponent {
  ...
  invia(c: Chat) {
    ...
    dx.messaggio = this.messaggio;
    dx.sessione = this.sessione;
    dx.destinatario = c.nickname;

    // richiesta http
    let oss: Observable<RegistrazioneDto> =
      this.http.post<RegistrazioneDto>('http://localhost:8080/inviaUno01', dx);
  ...
  aggiorna() {
    let dx: RichiediMessaggiDto = new RichiediMessaggiDto();

    dx.sessione = this.sessione;

    // richiesta http
    let oss: Observable<RegistrazioneDto> =
      this.http.post<RegistrazioneDto>('http://localhost:8080/aggiorna01', dx);
